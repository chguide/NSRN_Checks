    ...
    if int(arcpy.GetCount_management(errorpts).getOutput(0)) > 0:
        ...
        for row in scur:
            ...
            for rdrow in roadcur:
                rd1 = rdrow.getValue('ROADCLASS')
                chklist.append(str(rd1))
                print "ROADCLASS: " + str(rd1)
            if chklist[0] == chklist[1]:
                ...
                Append_management(errorpts_lyr, ws + os.sep + 'FalseErrors' + os.sep + 'Road_Class', 'NO_TEST')
...
##    SelectLayerByLocation_management(errorpts_lyr, 'ARE_IDENTICAL_TO', ws + os.sep + 'FalseErrors' + os.sep + 'Road_Names')
##    CalculateField_management(errorpts_lyr,"FalseErrorType","Road Name","VB","#")
##    SelectLayerByAttribute_management(errorpts_lyr,'CLEAR_SELECTION')
